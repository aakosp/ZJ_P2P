...
public class PtrFrameLayout extends ViewGroup {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (DEBUG && DEBUG_LAYOUT) {
            LogUtil.d(LOG_TAG, "onMeasure frame: width: %s, height: %s, padding: %s %s %s %s",
        ...
        if (mContent != null) {
            ...
            if (DEBUG && DEBUG_LAYOUT) {
                ...
                LogUtil.d(LOG_TAG, "onMeasure content, width: %s, height: %s, margin: %s %s %s %s",
                        getMeasuredWidth(), getMeasuredHeight(),
                        lp.leftMargin, lp.topMargin, lp.rightMargin, lp.bottomMargin);
                LogUtil.d(LOG_TAG, "onMeasure, currentPos: %s, lastPos: %s, top: %s",
    ...
    private void layoutChildren() {
        ...
        if (mHeaderView != null) {
            ...
            if (DEBUG && DEBUG_LAYOUT) {
                LogUtil.d(LOG_TAG, "onLayout header: %s %s %s %s", left, top, right, bottom);
            }
        }
        if (mContent != null) {
            if (isPinContent()) {
            ...
            if (DEBUG && DEBUG_LAYOUT) {
                LogUtil.d(LOG_TAG, "onLayout content: %s %s %s %s", left, top, right, bottom);
            }
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (DEBUG) {
                    ...
                    LogUtil.v(LOG_TAG, "ACTION_MOVE: offsetY:%s, currentPos: %s, moveUp: %s, canMoveUp: %s, moveDown: %s: canMoveDown: %s", offsetY, mPtrIndicator.getCurrentPosY(), moveUp, canMoveUp, moveDown, canMoveDown);
                }
    ...
    private void updatePos(int change) {
        ...
        if ((mPtrIndicator.hasJustLeftStartPosition() && mStatus == PTR_STATUS_INIT) ||
            ...
            if (DEBUG) {
                LogUtil.i(LOG_TAG, "PtrUIHandler: onUIRefreshPrepare, mFlag %s", mFlag);
            }
        }
        ...
        if (DEBUG) {
            LogUtil.v(LOG_TAG, "updatePos: change: %s, current: %s last: %s, top: %s, headerHeight: %s",
    ...
    final public void refreshComplete() {
        ...
        if (delay <= 0) {
            ...
            if (DEBUG) {
                LogUtil.d(LOG_TAG, "performRefreshComplete after delay: %s", delay);
            }
        }
    }
    ...
    private void performRefreshComplete() {
        ...
        if (mScrollChecker.mIsRunning && isAutoRefresh()) {
            // do nothing
            if (DEBUG) {
                LogUtil.d(LOG_TAG, "performRefreshComplete do nothing, scrolling: %s, auto refresh: %s",
    ...
    class ScrollChecker implements Runnable {
        ...
        private void finish() {
            if (DEBUG) {
                LogUtil.v(LOG_TAG, "finish, currentPos:%s", mPtrIndicator.getCurrentPosY());
            }
            reset();
            onPtrScrollFinish();
        }
        ...
        public void tryToScrollTo(int to, int duration) {
            ...
            if (DEBUG) {
                LogUtil.d(LOG_TAG, "tryToScrollTo: start: %s, distance:%s, to:%s", mStart, distance, to);
            }
            removeCallbacks(this);
